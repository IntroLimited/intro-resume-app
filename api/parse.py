...
def find_notion_candidate(candidate_name):
    ...
    for term in [candidate_name] + parts:
        try:
            data = notion_request("POST", f"/databases/{NOTION_DATABASE_ID}/query", {
                "filter": {"property": "Name", "title": {"contains": term}}
            })
            ...
            for page in results:
                ...
                page_name = "".join(t.get("plain_text", "") for t in name_prop.get("title", [])).lower()
                score = sum(1 for w in candidate_name.lower().split() if w in page_name)
                if score > best_score:
